    ...
    def extract_text_from_plate(self, plate_image_np):
        ...
        try:
            ...
            if final_cleaned_text and len(final_cleaned_text) >= 2:
                ...
                # Define absolute path for debug image directory
                script_dir = os.path.dirname(os.path.abspath(__file__))
                debug_image_dir = os.path.join(script_dir, "ocr_debug_images")

                # Log directory creation attempt
                print(f"OCR DEBUG: Attempting to create or ensure directory exists: {debug_image_dir}")
                os.makedirs(debug_image_dir, exist_ok=True)
                print(f"OCR DEBUG: Directory check complete for: {debug_image_dir}")
                ...
                try:
                    if gray_plate is not None:
                        ...
                        print(f"OCR DEBUG: Attempting to save debug image to: {full_debug_path}")
                        cv2.imwrite(full_debug_path, gray_plate)
                        print(f"OCR DEBUG: Successfully saved preprocessed plate image for Tesseract to {full_debug_path}")
                        saved_debug_path_status = full_debug_path
                    else:
                        print("OCR DEBUG: gray_plate not available for saving (was None).")
                        saved_debug_path_status = "NotAvailable (gray_plate was None)"
                except Exception as e_save:
                    print(f"OCR DEBUG: Error saving debug image: {str(e_save)}")
                    saved_debug_path_status = f"SaveError ({str(e_save)})"

                # Log failure after trying all PSM modes, using details from the last attempt
                print(f"OCR: All PSM modes failed. Text too short or invalid. Last Original: '{last_original_text}', Last Cleaned: '{last_cleaned_attempt}', Length: {len(last_cleaned_attempt)}, Plate Dims (Original Crop): {plate_image_np.shape[:2]}, Saved Debug Input: {saved_debug_path_status}")
                return None
